...
struct Grid {
    cell_size: f32,
    entities: Vec<(String, (usize, usize))>,  // Changed to store image paths instead of colors
...
impl Grid {
    ...
    fn render_entities(&mut self, commands: &mut Commands, asset_server: Res<AssetServer>) {
        ...
        // First render the background grid
        for row in 0..ROWS {
            for col in 0..COLUMNS {
                ...
                let entity = commands.spawn(
                    SpriteBundle {
                        ...
                        sprite: Sprite {
                            color: Color::rgb(0.1, 0.1, 0.1), // Dark background for game
        ...
        for (image_path, (row, col)) in &self.entities {
            ...
            let entity = commands.spawn((
                Node {
                    height: Val::Px(self.cell_size - 4.0),
                    width: Val::Px(self.cell_size - 4.0),
                    ..default()
                },
