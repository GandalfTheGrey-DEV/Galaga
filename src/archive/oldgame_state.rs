        ...
        for (cords, shipid) in to_update {
            if let Some(mut current_ship) = self.game_board.remove(&cords) {
                ...
                match current_ship.get_action(cords, &mut self.game_board) {
                    ...
                    ShipAction::Move(new_cords, wrapped) => {//TODO check if the ship did wrap with the bool remove all the code here thats not being used
                        let ship_can_wrap = current_ship.wrap();
                        let nx = cords.0 as i32 + change_x as i32;
                        let ny = cords.1 as i32 + change_y as i32;

                        let new_cords = if ship_can_wrap {
                            Some((
                                nx.rem_euclid(ROWS as i32) as usize,
                                ny.rem_euclid(COLUMNS as i32) as usize,
                            ))
                        } else {
                            if nx < 0 || nx >= ROWS as i32 || ny < 0 || ny >= COLUMNS as i32 {
                                ...
                                Some((
                                    nx.clamp(0, ROWS as i32 - 1) as usize,
                                    ny.clamp(0, COLUMNS as i32 - 1) as usize,
                                ))
                            }
                        };
        ...
        if let Some(player_pos) = self.player.current_position {
            if self.game_board.get(&player_pos).is_some() {
                ...
                if let Some(lives) = self.player.handle_collision() {
                    println!("oh crap, lives left: {}", lives);
                } else {
                    println!("ow, you died.");
                    exit(0);
                }
